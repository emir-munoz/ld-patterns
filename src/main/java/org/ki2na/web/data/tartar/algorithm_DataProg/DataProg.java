package org.ki2na.web.data.tartar.algorithm_DataProg;
...
import org.ki2na.web.data.tartar.common.SyntacticCategorization;
...
public class DataProg
{
	...
	private String getKeyValueOfTokenObject(Object tokenObject)
	{
		...
		if (tokenObject instanceof Integer)
			s = sc.getNameOfSyntacticCategory(((Integer) tokenObject).intValue());
		else
			s = ((String) tokenObject).toLowerCase();
	...
	public ArrayList<String> getSignificantPatterns(double treshold)
	{
		...
		if (treshold > 0.)
		{
			...
			/*
			 * ArrayList newlist = new ArrayList(); for (int i=0; i<list.size(); i++) { String s =
			 * (String)list.get(i); if (getCoverageOfPattern(s)>=treshold) newlist.add(s); } list =
			 * newlist;
			 */
		}
		return list;
